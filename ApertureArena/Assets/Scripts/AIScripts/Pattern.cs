...
{
    public abstract float Execute(BulletHellAI ai, Vector3 forward, float bulletSpeed, float patternSpeed);
}
public class Nova : Pattern
{
    //This pattern loops once so pS doesn't effect it
    public override float Execute(BulletHellAI ai, Vector3 forward,  float bS, float pS) {
      Quaternion rotation;
      for (int i = 0; i < 16; i++) {
